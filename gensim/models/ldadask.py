...
class LdaDask(interfaces.TransformationABC, basemodel.BaseTopicModel):
    ...
    def __init__(self, corpus=None, num_topics=100, id2word=None,
        ...
        if not distributed:
            ...
            logger.info("using distributed version with dask")
    ...
    def update(self, corpus, chunksize=None, decay=None, offset=None,
        ...
        print("update() called: ")
        print("\tchunks_as_numpy={}".format(chunks_as_numpy))
...
def dask_inference(chunk, expElogbeta, num_topics, iterations, alpha,
                   gamma_threshold, dtype, random_state):
    """Given a chunk of sparse document vectors, estimate gamma (parameters controlling the topic weights)
    for each document in the chunk.
    ...
    if len(chunk) > 1:
        logger.debug("%i/%i documents converged within %i iterations", converged, len(chunk), iterations)
...
def dask_do_estep(chunk, state, expElogbeta, num_topics, iterations, alpha,
                  gamma_threshold, dtype, random_state):
    """Perform inference on a chunk of documents, and accumulate the collected sufficient statistics.
    ...
    """
    gamma, sstats = dask_inference(chunk, expElogbeta, num_topics, iterations, alpha,
                                   gamma_threshold, dtype, random_state)
    state.sstats += sstats
    state.numdocs += gamma.shape[0]  # avoids calling len(chunk) on a generator
    assert gamma.dtype == dtype
